...
async function scenario(store,suffix){
  ...
  const profile2=await store.upsertCoachingProfile(user.id,JSON.stringify({version:1,goal:"muscle_gain"}),now+3,1);
  ...
  const log1=await store.upsertCoachingDailyLog({userId:user.id,logDate:"2030-03-04",calories:2100,proteinG:null,carbsG:null,fatG:null,updatedAt:now+6},0);
  const staleLog=await store.upsertCoachingDailyLog({userId:user.id,logDate:"2030-03-04",calories:9999,proteinG:null,carbsG:null,fatG:null,updatedAt:now+7},0);
  const log2=await store.upsertCoachingDailyLog({userId:user.id,logDate:"2030-03-04",calories:2200,proteinG:160,carbsG:250,fatG:65,updatedAt:now+8},1);
  ...
  return {
    ...
    week:{weekStart:week.week_start,planKey:week.plan_key,profileRevision:Number(week.profile_revision)},
    log1:{calories:Number(log1.calories),revision:Number(log1.revision)},staleLog,
    log2:{calories:Number(log2.calories),proteinG:Number(log2.protein_g),revision:Number(log2.revision)},
    listed:(await store.coachingDailyLogs(user.id,"2030-03-01","2030-03-10")).length,
    exported:{profile:JSON.parse(exportRows.coachingProfile.profile_json),weeks:exportRows.coachingWeeks.length,logs:exportRows.coachingLogs.length},
...
test("SQLite and Turso keep coaching revisions, exports, and deletion behavior identical",{concurrency:false},async()=>{
  ...
  try{
    ...
    assert.deepEqual(local.week,{weekStart:"2030-03-04",planKey:"week-key",profileRevision:2});assert.equal(local.staleLog,null);assert.deepEqual(local.log2,{calories:2200,proteinG:160,revision:2});
    assert.deepEqual(local.exported,{profile:{version:1,goal:"muscle_gain"},weeks:1,logs:1});assert.equal(local.deleted,"deleted");assert.deepEqual(local.after,{profile:null,week:null,log:null});
