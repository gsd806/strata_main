...
test("both food surfaces render scaled amounts and actual residuals for limited menus",async()=>{
  ...
  for(const prefix of ["coachingFood","progressCoachingFood"]){
...
test("calorie-only food options do not present unlogged macros as zero",async()=>{
  ...
  for(const prefix of ["coachingFood","progressCoachingFood"]){assert.match(element(`${prefix}Options`).innerHTML,/Macro differences are unavailable/);assert.equal(element(`${prefix}Remaining`).children[1].textContent,"—");assert.ok(element(`${prefix}Remaining`).macros.every((node)=>node.hidden));}
});
...
test("late food results cannot restore private ingredients after reset",async()=>{
  ...
  for(const prefix of ["coachingFood","progressCoachingFood"]){assert.equal(element(`${prefix}Options`).textContent,"");assert.equal(element(`${prefix}Options`).innerHTML,"");assert.ok(element(`${prefix}Remaining`).children.every((node)=>node.textContent==="—"));assert.equal(element(`${prefix}Refresh`).disabled,true);}
});
...
test("switching to a historical diary date clears stale current-week food results",async()=>{
  ...
  for(const prefix of ["coachingFood","progressCoachingFood"]){assert.equal(element(`${prefix}Options`).innerHTML,"");assert.equal(element(`${prefix}Refresh`).disabled,true);assert.match(element(`${prefix}Status`).textContent,/Historical intake/);assert.ok(element(`${prefix}Remaining`).children.every((node)=>node.textContent==="—"));}
});
