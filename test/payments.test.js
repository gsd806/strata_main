...
test("transaction creation fixes catalog and account metadata on the server",async()=>{
  ...
  const fetchImpl=async(url,options)=>{
    ...
    return {ok:true,json:async()=>({data:{id:transactionId,status:"draft"}})};
  };
  const account={
  ...
  assert.deepEqual(body,{
    ...
    collection_mode:"automatic",
    custom_data:{strata_user_id:"user-server-owned",strata_version:1}
  });
...
test("transaction creation fails closed with sanitized errors",async(t)=>{
  ...
  await t.test("network failure",async()=>{
    ...
    await assert.rejects(
      createPaddleTransaction(config,{userId:"user-1"},async()=>{throw new Error(upstream);}),
      (error)=>{
