...
| `src/coaching-core.js` | Strict versioned profile/log validation plus deterministic training-week composition. Profile version 3 is the explicit energy-semantics transition. |
| `src/energy-planning-core.js` | Pure version-dispatched legacy calculation, 2023 DRI EER estimation, bounded version-3 trend calibration, calorie-pattern, macro, and weight-scenario policy. |
...
| `src/migrations.js` | Ordered, idempotent SQLite and Turso migration ledger, including security, index, active-workout, recurring-subscription, and coaching-evidence migrations. |
...
The profile boundary accepts an exact, versioned field set: measurement units, height, weight, optional body-fat percentage, the energy-equation coefficient, activity answer, goal and pace, experience, available days and session duration, equipment, movement limitations, optional known-exercise capabilities, calorie pattern, optional macro preference, time zone, and one nested `mealPreferences` object. Profile version is authoritative calculation state. Stored version 1 and version 2 keep the exact pre-7.10 resting-energy × legacy activity-factor semantics and remain readable without mutation; reads, week rollover, and deployment never reinterpret those answers. New setup and every explicit profile review/save write version 3, require the adult 2023 EER range of ages 19–80 plus a published equation coefficient, and interpret activity as a whole-day PAL category. The meal object records an explicit allergy state, supported allergen tags, a bounded other-or-uncertain-allergy note, dietary pattern, gluten-free or dairy-free requirements, favorite-food categories, a preferred 1–6 meals per day, and an optional whole-cent daily USD budget. Unknown fields, repeated days or movements, invalid catalog IDs, contradictory allergy states, impossible units, and out-of-range values are rejected. Entered sets, repetitions, and load are treated as unverified capability context and only cap the first-week prescription conservatively; they are not interpreted as tested one-repetition maximums.

A weekly snapshot is generated from the canonical profile and its semantic version, revision, the Monday in the saved time zone, the exercise and meal catalog fingerprints, the generator and energy-model versions, and a bounded summary of observations from the 21 days before that Monday. Build 7.10's `energy-planning-v2` identifier invalidates incompatible cached generation, but it cannot upgrade a profile or select version-3 semantics by itself. The same week and profile revision reuse the persisted snapshot; completing a diary entry does not move the current week's targets. Editing the profile deliberately creates a new current-week snapshot. In particular, reviewing and saving a version-1/version-2 profile writes version 3 and makes the model change explicit. The following Monday changes the key and rotates at least one eligible unconstrained movement. Exercise selection respects experience, session duration, equipment, and saved movement limitations and returns a reviewable constraint error when it cannot build a complete session. The snapshot stores its canonical profile inputs—including the profile version and nested food preferences—plus the calibration cutoff, counts, decision, assumptions, references, and safety caveats alongside the result. This private duplication keeps the generation evidence reviewable; it does not create a public profile. The detailed formulas and limits are recorded in [the coaching methodology](coaching-methodology.md).

Energy output exposes which profile semantics produced it. Version 1 and version 2 retain the exact earlier resting-energy × stored activity-factor baseline, return the Legacy profile calibration state with zero adjustment, and do not reinterpret the stored activity choice. Version 3 separates the 2023 adult DRI total-expenditure baseline, visible Mifflin–St Jeor and optional Cunningham cross-checks, and trend-informed maintenance when evidence qualifies. Body-fat input never silently replaces the primary version-3 EER target. Its all-day activity category already includes exercise, so the server does not add generated-workout calories again. Steady, training-day zigzag, and flexible-day patterns redistribute one exact weekly calorie budget, and a pattern falls back to steady if variation would cross the conservative 1,200 kcal floor. Macros are optional. Weight values are broad 4-, 8-, and 12-week planning scenarios with uncertainty bands, not promised outcomes.

Daily writes are limited to the current coaching week and store calories, either all three macros or none, an optional morning weight, and a nullable complete-day state. Null preserves a legacy row's unknown completeness; only an explicit true value can qualify as version-3 calibration evidence. Before generation, the service requests only the preceding 21-day owner/date range. The pure energy leaf may expose evidence counts for a legacy profile but must return zero adjustment. For version 3 it requires 18 complete calorie days and 12 valid weights spanning at least 14 calendar days, derives a robust trend, rejects outliers and contradictory estimates, then shrinks and caps any change. Profile and log revisions use compare-and-swap semantics so a stale tab receives the latest safe state for explicit review instead of overwriting it. The existing `(user_id, log_date)` primary key is the actual evidence lookup path, so no secondary index is added.
