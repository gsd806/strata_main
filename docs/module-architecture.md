...
The command-generated table below is the Build 7.2.0 review snapshot. CI generates the same table on every architecture check, while the policy enforces budgets and edges against the live sources.
...
| `src/server.js` | HTTP composition root | 1172 | 1133 | 59.7 KiB | 1200 | `src/admin.js`, `src/auth.js`, `src/database.js`, `src/email.js`, `src/http.js`, `src/payments.js`, `src/plans.js`, `src/service-composition.js`, `src/setup.js`, `src/static-assets.js`, `src/support.js`, `src/workouts.js` |
