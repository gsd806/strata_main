...
For an activity lasting `m` minutes, gross energy is estimated as `MET × reference oxygen × kg / 200 × m`. The adult branch uses `3.5 mL/kg/min` and the age-60-or-older branch uses `2.7 mL/kg/min`, consistent with the reference conventions in the [2024 Adult Compendium](https://pubmed.ncbi.nlm.nih.gov/38242596/) and [2024 Older Adult Compendium](https://pubmed.ncbi.nlm.nih.gov/38242593/). General resistance training uses `3.5 MET` for adults and `4.3 MET` for older adults. STRATA subtracts `Mifflin RMR / 1440 × m` because that resting time is already in the non-workout base. MET values are standardized estimates, not wearable measurements, and can be inaccurate for a particular exercise, pace, body composition, or person.
