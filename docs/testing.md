...
## Build 7.8.1 additions

Focused unit tests keep `StrataParticleChart` bounded and reusable: they cover its STRATA theme, reduced-motion option, create-versus-update behavior, render failure, resize, and synchronous destruction. Adapter tests separately protect Strata+’s exact movement/measurement/load/unit grouping and latest-12 window, Planner’s top-eight primary-muscle set projection over unchanged exact rows, and Workout History’s exact comparable series across the currently loaded window. A single workout point remains a baseline without a trend canvas in Strata+ and Workout History.

Runtime and real-browser checks prove that each successful render has one chart root, selector changes update rather than append, exact HTML values remain available, and a missing chart runtime leaves useful fallback content. The browser journeys also measure chart, controls, and internally scrolling exact tables at 320 px and verify reduced-motion behavior. Private Strata+ and Workout checks require synchronous instance destruction and removal of canvas, generated accessibility DOM, selectors, status, scope, and exact values before an account identity can change.

Supply-chain and PWA tests pin the byte-identical same-origin Particle Charts 1.0.0 asset, its checksum and SRI, the complete MIT notice, public static allowlisting, dependency order, and build-versioned precache entries. They also continue to prove that private workout API responses and rendered chart values never enter Cache Storage or browser persistence.
