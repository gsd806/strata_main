# STRATA 7.5.0 readiness

Status: source candidate under final release verification; not deployed.

Build 7.5.0 is the Training Memory and operational-trust release. It preserves a complete guest week through account activation, adds evidence-based workout memory and deliberate workout-versus-Plan changes, permits bounded offline continuation of an already-open workout, makes planner and block review actionable, adds account session/export controls, moves new Strata+ sales to a $0.99 USD monthly subscription with one optional no-card 30-minute trial, and strengthens the architecture, migration, observability, deployment, asset-privacy, accessibility, and browser boundaries. See the [release guide](release-7.5.0.md) and [changelog](../CHANGELOG.md).

## Verification record

| Check | Current result |
| --- | --- |
| Complete release gate | Pending final integrated run |
| Node regression tests | 553 passed, zero failed |
| Coverage | 93.74% lines; 79.98% branches; 89.46% functions; enforced 90% / 78% / 85% floors passed |
| Architecture | Passed; 32 modules, zero dependency cycles, zero policy violations; exact table in [module architecture](module-architecture.md) |
...
The local performance gate also passed on that Darwin/Node host. Median/p95 latency was 0.370/0.596 ms for health readiness, 0.310/0.956 ms for public status, 0.320/0.445 ms for authenticated Plan reads, and 0.494/0.799 ms for Plan saves. Direct storage medians/p95s were 0.008/0.011 ms for session lookup, 0.004/0.006 ms for Plan lookup, and 0.042/0.051 ms for Plan compare-and-swap. Each result used 8 warm-ups followed by 40 measured samples. These local, fixture-backed timings are regression evidence rather than hosted Turso, Internet, or provider capacity evidence.
...
Migration `004-monthly-subscriptions` is ordered and idempotent in both adapters. It adds a nullable subscription link and lean subscription cache while retaining legacy lifetime rows. The architecture report shows the HTTP root reduced from 1,185 to 732 lines and the dual adapter from 1,315 to 1,135 through downward billing, account-self-service, schema, and store edges—not a cosmetic file split.
