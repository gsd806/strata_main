...
const COACHING_SQL=Object.freeze({
  ...
  upsertCoachingWeek:"INSERT INTO coaching_weeks(user_id,week_start,plan_key,profile_revision,snapshot_json,generated_at) SELECT u.id,?,?,?,?,? FROM users u JOIN coaching_profiles p ON p.user_id=u.id AND p.revision=? WHERE u.id=? AND u.suspended_at IS NULL ON CONFLICT(user_id,week_start) DO UPDATE SET plan_key=excluded.plan_key,profile_revision=excluded.profile_revision,snapshot_json=excluded.snapshot_json,generated_at=excluded.generated_at WHERE coaching_weeks.plan_key<>excluded.plan_key OR coaching_weeks.profile_revision<>excluded.profile_revision RETURNING week_start,plan_key,profile_revision,snapshot_json,generated_at",
  coachingDailyLog:"SELECT log_date,calories,protein_g,carbs_g,fat_g,morning_weight_kg,intake_complete,revision,updated_at FROM coaching_daily_logs WHERE user_id=? AND log_date=?",
  coachingDailyLogs:"SELECT log_date,calories,protein_g,carbs_g,fat_g,morning_weight_kg,intake_complete,revision,updated_at FROM coaching_daily_logs WHERE user_id=? AND log_date>=? AND log_date<=? ORDER BY log_date",
