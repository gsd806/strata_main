...
const SQL = {
  ...
  deleteUserByAdmin:`DELETE FROM users WHERE id=? AND email COLLATE BINARY=? AND suspended_at IS NOT NULL AND NOT EXISTS (SELECT 1 FROM admin_principal ap WHERE ap.user_id=users.id) AND NOT EXISTS (SELECT 1 FROM paddle_purchases p WHERE p.user_id=users.id AND ${BILLING_DELETION_BLOCKER}) AND NOT EXISTS (SELECT 1 FROM paddle_checkout_claims c WHERE c.user_id=users.id AND c.expires_at>?) AND EXISTS (SELECT 1 FROM admin_principal ap JOIN users actor ON actor.id=ap.user_id JOIN sessions s ON s.user_id=actor.id AND s.auth_version=actor.auth_version WHERE ap.slot='primary' AND ap.user_id=? AND ap.configured_email=actor.email COLLATE NOCASE AND actor.email_verified_at IS NOT NULL AND actor.suspended_at IS NULL AND s.token_hash=? AND s.expires_at>?) RETURNING id,email`,
