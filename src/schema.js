...
const COMPLETED_WORKOUT_FILTER="CASE WHEN json_valid(summary_json) THEN json_extract(summary_json,'$.status') END='completed'";
const SUBSCRIPTION_RENEWAL_WINDOW_MS=32*24*60*60*1000;
const {ADMIN_CONTROLS_TABLE,ACCESS_CONTROLS_SQL,CONTROL_COLUMNS,activeGrant}=require("./access-controls-schema");
...
const SQL = {
  ...
  adminOverview:withEntitlementClock(`SELECT (SELECT COUNT(*) FROM users) AS total_users,(SELECT COUNT(*) FROM users WHERE email_verified_at IS NOT NULL) AS verified_users,(SELECT COUNT(*) FROM users WHERE suspended_at IS NOT NULL) AS suspended_users,(SELECT COUNT(*) FROM sessions s JOIN users u ON u.id=s.user_id AND u.auth_version=s.auth_version WHERE s.expires_at>? AND u.suspended_at IS NULL) AS active_sessions,(SELECT COUNT(*) FROM users u WHERE u.suspended_at IS NULL AND (EXISTS(SELECT 1 FROM paddle_purchases pp WHERE pp.user_id=u.id AND ${activeEntitlement("pp")}) OR EXISTS(SELECT 1 FROM discovery_trials t WHERE t.user_id=u.id AND t.expires_at>(SELECT now FROM entitlement_clock)) OR EXISTS(SELECT 1 FROM admin_account_controls ac WHERE ac.user_id=u.id AND ${activeGrant("ac")}))) AS discovery_users,(SELECT COUNT(*) FROM paddle_purchases WHERE paddle_status<>'canceled' AND completed_at IS NULL AND access_revoked_at IS NULL) AS pending_payments,(SELECT COUNT(*) FROM account_action_requests WHERE purpose='account_delete' AND delivery_state='sent' AND consumed_at IS NULL AND expires_at>?) AS pending_deletions,(SELECT COUNT(*) FROM support_tickets WHERE status<>'resolved') AS open_support,(SELECT COUNT(*) FROM (SELECT user_id FROM workouts WHERE ${COMPLETED_WORKOUT_FILTER} GROUP BY user_id)) AS first_workout_users,(SELECT COUNT(*) FROM (SELECT user_id FROM workouts WHERE ${COMPLETED_WORKOUT_FILTER} GROUP BY user_id HAVING COUNT(*)>=2)) AS second_workout_users,(SELECT COUNT(*) FROM (SELECT user_id FROM workouts WHERE ${COMPLETED_WORKOUT_FILTER} GROUP BY user_id HAVING MAX(started_at)-MIN(started_at)>=604800000)) AS day_eight_return_users,(SELECT COUNT(DISTINCT user_id) FROM discovery_trials) AS trial_users,(SELECT COUNT(DISTINCT user_id) FROM paddle_purchases WHERE completed_at IS NOT NULL) AS paid_users,(SELECT COUNT(*) FROM paddle_subscriptions WHERE current_period_ends_at>created_at+${SUBSCRIPTION_RENEWAL_WINDOW_MS}) AS renewed_subscriptions`),
  adminUserById:withEntitlementClock(`SELECT ${CONTROL_COLUMNS},(SELECT expires_at FROM discovery_trials t WHERE t.user_id=u.id AND t.expires_at>(SELECT now FROM entitlement_clock)) AS trial_expires_at,u.id,u.name,u.email,u.created_at,u.email_verified_at,u.auth_version,u.suspended_at,p.plan_json,(SELECT COUNT(*) FROM sessions s WHERE s.user_id=u.id AND s.auth_version=u.auth_version AND s.expires_at>?) AS active_session_count,(SELECT COUNT(*) FROM ratings r WHERE r.user_id=u.id) AS rating_count,(SELECT COUNT(*) FROM paddle_purchases pp WHERE pp.user_id=u.id) AS purchase_count,(SELECT COUNT(*) FROM paddle_purchases pp WHERE pp.user_id=u.id AND ${activeEntitlement("pp")}) AS active_purchase_count,(SELECT COUNT(*) FROM paddle_purchases pp WHERE pp.user_id=u.id AND pp.paddle_status<>'canceled' AND pp.completed_at IS NULL AND pp.access_revoked_at IS NULL) AS pending_purchase_count,(SELECT MAX(pp.updated_at) FROM paddle_purchases pp WHERE pp.user_id=u.id) AS latest_purchase_at,(SELECT pp.transaction_id FROM paddle_purchases pp WHERE pp.user_id=u.id ORDER BY pp.updated_at DESC,pp.transaction_id DESC LIMIT 1) AS transaction_id,(SELECT pp.paddle_status FROM paddle_purchases pp WHERE pp.user_id=u.id ORDER BY pp.updated_at DESC,pp.transaction_id DESC LIMIT 1) AS transaction_status,(SELECT request_id FROM account_action_requests a WHERE a.user_id=u.id AND a.purpose='account_delete' AND a.delivery_state='sent' AND a.consumed_at IS NULL AND a.expires_at>? LIMIT 1) AS deletion_request_id,(SELECT expires_at FROM account_action_requests a WHERE a.user_id=u.id AND a.purpose='account_delete' AND a.delivery_state='sent' AND a.consumed_at IS NULL AND a.expires_at>? LIMIT 1) AS deletion_expires_at FROM users u LEFT JOIN admin_account_controls ac ON ac.user_id=u.id LEFT JOIN plans p ON p.user_id=u.id WHERE u.id=?`),
  adminUsers:withEntitlementClock(`SELECT ${CONTROL_COLUMNS},(SELECT expires_at FROM discovery_trials t WHERE t.user_id=u.id AND t.expires_at>(SELECT now FROM entitlement_clock)) AS trial_expires_at,u.id,u.name,u.email,u.created_at,u.email_verified_at,u.suspended_at,(SELECT COUNT(*) FROM sessions s WHERE s.user_id=u.id AND s.auth_version=u.auth_version AND s.expires_at>?) AS active_session_count,(SELECT COUNT(*) FROM paddle_purchases pp WHERE pp.user_id=u.id) AS purchase_count,(SELECT COUNT(*) FROM paddle_purchases pp WHERE pp.user_id=u.id AND ${activeEntitlement("pp")}) AS active_purchase_count,(SELECT COUNT(*) FROM paddle_purchases pp WHERE pp.user_id=u.id AND pp.paddle_status<>'canceled' AND pp.completed_at IS NULL AND pp.access_revoked_at IS NULL) AS pending_purchase_count,(SELECT MAX(pp.updated_at) FROM paddle_purchases pp WHERE pp.user_id=u.id) AS latest_purchase_at,(SELECT pp.transaction_id FROM paddle_purchases pp WHERE pp.user_id=u.id ORDER BY pp.updated_at DESC,pp.transaction_id DESC LIMIT 1) AS transaction_id,(SELECT pp.paddle_status FROM paddle_purchases pp WHERE pp.user_id=u.id ORDER BY pp.updated_at DESC,pp.transaction_id DESC LIMIT 1) AS transaction_status,(SELECT expires_at FROM account_action_requests a WHERE a.user_id=u.id AND a.purpose='account_delete' AND a.delivery_state='sent' AND a.consumed_at IS NULL AND a.expires_at>? LIMIT 1) AS deletion_expires_at FROM users u LEFT JOIN admin_account_controls ac ON ac.user_id=u.id WHERE (?='' OR lower(u.name) LIKE ? ESCAPE '\\' OR lower(u.email) LIKE ? ESCAPE '\\' OR lower(u.id) LIKE ? ESCAPE '\\' OR EXISTS (SELECT 1 FROM paddle_purchases pp WHERE pp.user_id=u.id AND lower(pp.transaction_id) LIKE ? ESCAPE '\\')) ORDER BY u.created_at DESC,u.id DESC LIMIT ? OFFSET ?`),
  ...
  deleteUserByAdmin:`DELETE FROM users WHERE id=? AND email COLLATE BINARY=? AND suspended_at IS NOT NULL AND NOT EXISTS (SELECT 1 FROM admin_principal ap WHERE ap.user_id=users.id) AND NOT EXISTS (SELECT 1 FROM paddle_purchases p WHERE p.user_id=users.id AND ${BILLING_DELETION_BLOCKER}) AND NOT EXISTS (SELECT 1 FROM paddle_checkout_claims c WHERE c.user_id=users.id AND c.expires_at>?) AND EXISTS (SELECT 1 FROM admin_principal ap JOIN users actor ON actor.id=ap.user_id JOIN sessions s ON s.user_id=actor.id AND s.auth_version=actor.auth_version JOIN admin_elevations ae ON ae.session_token_hash=s.token_hash WHERE ap.slot='primary' AND ap.user_id=? AND ap.configured_email=actor.email COLLATE NOCASE AND actor.email_verified_at IS NOT NULL AND actor.suspended_at IS NULL AND s.token_hash=? AND s.expires_at>? AND ae.expires_at>?) RETURNING id,email`,
