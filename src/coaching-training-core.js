...
const MODEL_VERSION="coaching-training-v1";
...
/** @param {Value[]} items */
function totalSeconds(items){return 300+items.reduce((total,item)=>total+estimatedSeconds(item),0);}
...
function fitDuration(items,minutes){
  ...
  if(totalSeconds(items)>minutes*60)throw constraintError("The required exercises and rests do not fit this time budget. Choose a longer workout or review the equipment and exercise restrictions.");
...
function buildTraining(profile,weekStart,evidence={}){
  ...
  const sessions=split.map((template,index)=>{
    ...
    const used=new Set(),items=/** @type {Value[]} */([]);
    for(const [slot,role] of template.roles.slice(0,count).entries()){
      const exercise=selectExercise(profile,role,used,{evidence:normalized,day,label:template.label,index,weekStart});
      if(!exercise){if(slot<4&&PRIMARY_ROLES.has(role)&&!items.some(item=>item.role===role))throw constraintError(`No compatible ${LABELS[role]||role} exercise fits the selected equipment and experience. Keep your actual experience level and movement limits; choose compatible equipment or a different workout setup.`);continue;}
      const format=exerciseFormat(exercise,profile.preferredLoadUnit);if(!format)continue;used.add(String(exercise.id));
    ...
    const duration=fitDuration(items,profile.sessionMinutes),exercises=items.map(item=>withPerformance(item,normalized,weekStart)),workingSets=exercises.reduce((sum,item)=>sum+item.sets,0),estimatedDurationMinutes=Math.ceil(totalSeconds(items)/60);
    return {day,date:new Date(Date.parse(`${weekStart}T00:00:00Z`)+DAYS.indexOf(day)*86400000).toISOString().slice(0,10),label:template.label,rationale:`${workingSets} working sets with ${goal==="strength"?"longer rests for strength-focused compound work":goal==="hypertrophy"?"moderate repetition ranges for muscle-focused training":"repeatable strength and muscle-building practice"}. About ${estimatedDurationMinutes} minutes including planned rests and setup.`,exercises,workingSets,estimatedDurationMinutes,availableMinutes:profile.sessionMinutes,durationAdjusted:duration.adjusted};
  });
