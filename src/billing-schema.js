...
const ENTITLED_RECURRING_CATALOG="subscription_id IS NULL OR (price_id IN (SELECT value FROM json_each(?)) AND product_id=?)";
...
const BILLING_SQL={
  ...
  updatePaddleSubscriptionAfterCatalog:`UPDATE paddle_subscriptions SET customer_id=?,status=?,price_id=?,product_id=?,scheduled_change_action=?,scheduled_change_at=?,current_period_ends_at=?,event_occurred_at=?,updated_at=? WHERE subscription_id=? AND user_id=? AND (event_occurred_at<? OR (event_occurred_at=? AND ${subscriptionStateRank("?")}>${subscriptionStateRank("status")})) AND EXISTS (SELECT 1 FROM paddle_purchases p WHERE p.transaction_id=paddle_subscriptions.transaction_id AND p.user_id=paddle_subscriptions.user_id AND p.subscription_id=paddle_subscriptions.subscription_id AND p.price_id=? AND p.product_id=?) RETURNING ${SUBSCRIPTION_COLUMNS}`,
  replaceSubscriptionPurchaseCatalog:`UPDATE paddle_purchases SET price_id=?,product_id=?,updated_at=MAX(updated_at,?) WHERE transaction_id=? AND user_id=? AND subscription_id=? AND price_id=? AND product_id=? AND paddle_status='completed' AND completed_at IS NOT NULL AND EXISTS (SELECT 1 FROM paddle_subscriptions s WHERE s.subscription_id=paddle_purchases.subscription_id AND s.transaction_id=paddle_purchases.transaction_id AND s.user_id=paddle_purchases.user_id AND (s.event_occurred_at<? OR (s.event_occurred_at=? AND ${subscriptionStateRank("?")}>${subscriptionStateRank("s.status")}))) RETURNING ${PURCHASE_COLUMNS}`,
