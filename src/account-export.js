...
function exportPayload(rows,now){
  ...
  return{
    ...
    coaching:{profile:rows.coachingProfile?{data:storedJson(rows.coachingProfile.profile_json),revision:Number(rows.coachingProfile.revision),updatedAt:Number(rows.coachingProfile.updated_at)}:null,weeks:(rows.coachingWeeks||[]).map((row)=>({weekStart:String(row.week_start),planKey:String(row.plan_key),profileRevision:Number(row.profile_revision),data:storedJson(row.snapshot_json),generatedAt:Number(row.generated_at)})),logs:(rows.coachingLogs||[]).map((row)=>({date:String(row.log_date),calories:Number(row.calories),proteinG:optionalNumber(row.protein_g),carbsG:optionalNumber(row.carbs_g),fatG:optionalNumber(row.fat_g),morningWeightKg:optionalNumber(row.morning_weight_kg),complete:row.intake_complete==null?null:Number(row.intake_complete)===1,revision:Number(row.revision),updatedAt:Number(row.updated_at)}))},
