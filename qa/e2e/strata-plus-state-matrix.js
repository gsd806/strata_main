...
test("the four Strata+ tabs retain keyboard focus, mobile identity, reduced motion, and a 31-day browser flow",{timeout:30_000},async(t)=>{
  ...
  assert.deepEqual(await primary.locator("span").allTextContents(),["Today","Plan","Progress","Explore"]);
  ...
  for(const [label,panel,heading] of [["Today","todayWorkspace","todayTitle"],["Plan","planWorkspace","planWorkspaceTitle"],["Progress","progressWorkspace","progressWorkspaceTitle"],["Explore","exploreWorkspace","exploreWorkspaceTitle"]]){
  ...
  assert.equal(await page.locator("#scoreGuide").isVisible(),true);assert.equal(await page.locator("#scoreGuideDetails").evaluate((node)=>node.open),false);assert.equal(await page.locator("#scoreGuide").count(),1);
