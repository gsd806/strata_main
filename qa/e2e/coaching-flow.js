...
test("a Strata+ member builds, tracks, reloads, and safely refreshes a coaching week",{timeout:60_000},async()=>{
  ...
  try{
    ...
    for(const width of [1440,768,390,320]){await page.setViewportSize({width,height:900});const layout=await page.evaluate(()=>({overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,calibrationOverflow:document.querySelector("#coachingCalibration").scrollWidth-document.querySelector("#coachingCalibration").clientWidth,logOverflow:document.querySelector("#coachingLogForm").scrollWidth-document.querySelector("#coachingLogForm").clientWidth}));assert.ok(layout.overflow<=1,`coaching dashboard overflows ${width}px by ${layout.overflow}px`);assert.ok(layout.calibrationOverflow<=1,`calibration panel overflows ${width}px by ${layout.calibrationOverflow}px`);assert.ok(layout.logOverflow<=1,`daily log overflows ${width}px by ${layout.logOverflow}px`);if([1440,390].includes(width)){const artifacts=join(ROOT,"test-runtime","coaching-screens");mkdirSync(artifacts,{recursive:true});await page.locator("#coachingCalibration").screenshot({path:join(artifacts,`calibration-${width}.png`)});await page.locator("#coachingFoodOptions .coaching-meal-card").first().screenshot({path:join(artifacts,`food-${width}.png`)});await page.locator("#coachingTrainingCoverage").screenshot({path:join(artifacts,`training-coverage-${width}.png`)});}}await page.setViewportSize({width:1280,height:900});
