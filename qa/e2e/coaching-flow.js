...
test("a Strata+ member builds, tracks, reloads, and safely refreshes a coaching week",{timeout:60_000},async()=>{
  ...
  try{
    ...
    await page.check('input[name="caloriePattern"][value="training_day"]');await page.check("#coachingMacrosEnabled");await page.check("#mealAllergyNone");await page.selectOption("#mealDietaryPattern","omnivore");await page.selectOption("#mealMealsPerDay","3");await page.fill("#mealDailyBudget","15.00");await page.check('input[name="mealFavorite"][value="chicken"]');await page.check('input[name="mealFavorite"][value="rice"]');
    ...
    for(const width of [768,390,320]){await page.setViewportSize({width,height:900});const overflow=await page.evaluate(()=>document.documentElement.scrollWidth-document.documentElement.clientWidth);assert.ok(overflow<=1,`food options overflow ${width}px by ${overflow}px`);}await page.setViewportSize({width:1280,height:900});
    ...
    await destinations.nth(2).click();await page.locator("#progressCalorieCard").waitFor({state:"visible"});assert.equal(await page.locator("#progressCoachingCaloriesEaten").inputValue(),"1800");assert.equal(await page.locator("#progressCoachingProteinEaten").inputValue(),"150");assert.equal(await page.locator("#progressCoachingFoodOptions .coaching-meal-card").count(),3);assert.match((await page.locator("#progressCalorieTitle").textContent())||"",/LOG [A-Z]+’S TOTAL/);assert.equal(await page.locator(".progress-calorie-form label > span").first().evaluate((node)=>globalThis.getComputedStyle(node).color),"rgb(16, 17, 15)");
    ...
    const purged=await page.evaluate(()=>({height:globalThis.document.querySelector("#coachingHeight")?.value,weight:globalThis.document.querySelector("#coachingWeight")?.value,capabilities:globalThis.document.querySelector("#coachingCapabilityRows")?.textContent,week:globalThis.document.querySelector("#coachingWeekGrid")?.textContent,calories:globalThis.document.querySelector("#coachingCaloriesEaten")?.value,projection:globalThis.document.querySelector("#coachingProjectionChart")?.textContent,foodOptions:globalThis.document.querySelector("#coachingFoodOptions")?.textContent,allergyChecked:[...globalThis.document.querySelectorAll('input[name="mealAllergyStatus"]')].some((input)=>input.checked),mainHidden:globalThis.document.querySelector("main")?.hidden}));
    assert.deepEqual(purged,{height:"",weight:"",capabilities:"",week:"",calories:"",projection:"",foodOptions:"",allergyChecked:false,mainHidden:true},"account revalidation must purge prior health, food-preference, and intake data before loading the next account");releaseDiscovery();await discoveryHandled;
