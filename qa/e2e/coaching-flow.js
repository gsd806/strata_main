...
test("a Strata+ member builds, tracks, reloads, and safely refreshes a coaching week",{timeout:60_000},async()=>{
  ...
  try{
    ...
    await page.selectOption("#coachingDailyActivity","moderately_active");await page.selectOption("#coachingGoal","deficit");await page.selectOption("#coachingGoalPace","gentle");await page.selectOption("#coachingExperience","intermediate");await page.fill("#coachingFrequency","3");await page.locator("#coachingFrequency").dispatchEvent("change");await page.selectOption("#coachingDuration","60");
    ...
    await page.check('input[name="caloriePattern"][value="training_day"]');await page.check("#coachingMacrosEnabled");await page.selectOption("#coachingMacroPreference","higher_protein");await page.check("#mealAllergyNone");await page.selectOption("#mealDietaryPattern","omnivore");await page.selectOption("#mealMealsPerDay","3");await page.fill("#mealDailyBudget","15.00");await page.check('input[name="mealFavorite"][value="chicken"]');await page.check('input[name="mealFavorite"][value="rice"]');
    ...
    const restingMethod=(await page.locator("#coachingBmrMethod").textContent())||"";assert.match(restingMethod,/Mifflin–St Jeor/i,"the resting estimate should name its primary resting-energy equation");assert.match(restingMethod,/Body-fat cross-check: about .* Cunningham 1991 .* noisy body-fat input/i,"body fat should remain a correctly attributed secondary cross-check");assert.match((await page.locator("#coachingTdeeDetail").textContent())||"",/2023 DRI/i,"maintenance should name the whole-day EER method");assert.match((await page.locator("#coachingMethodList").textContent())||"",/Cunningham resting-energy cross-check/i);assert.ok(await page.locator("#coachingProjectionChart svg[role=img]").count()===1);assert.match((await page.locator("#coachingProjectionNote").textContent())||"",/not promised outcomes/i);
    assert.match((await page.locator("#coachingCalibrationTitle").textContent())||"",/STARTING ESTIMATE/i);assert.equal((await page.locator("#coachingCalibrationIntake").textContent())?.trim(),"0 / 18");assert.equal((await page.locator("#coachingCalibrationWeights").textContent())?.trim(),"0 / 12 · 0 / 14 days");assert.match((await page.locator("#coachingCalibrationExplanation").textContent())||"",/21 days before a new coaching week/i);
    const foodOptionsResponse=await foodOptionsPromise;assert.equal(foodOptionsResponse.status(),200,await foodOptionsResponse.text());await page.locator("#coachingFoodOptions .coaching-meal-card").first().waitFor();assert.equal(await page.locator("#coachingFoodOptions .coaching-meal-card").count(),3);assert.match((await page.locator("#coachingFoodStatus").textContent())||"",/saved intake, targets, food preferences/i);assert.match((await page.locator("#coachingFoodOptions").textContent())||"",/Est\. cost/i);
    ...
    for(const width of [1440,768,390,320]){await page.setViewportSize({width,height:900});const layout=await page.evaluate(()=>({overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,calibrationOverflow:document.querySelector("#coachingCalibration").scrollWidth-document.querySelector("#coachingCalibration").clientWidth,logOverflow:document.querySelector("#coachingLogForm").scrollWidth-document.querySelector("#coachingLogForm").clientWidth}));assert.ok(layout.overflow<=1,`coaching dashboard overflows ${width}px by ${layout.overflow}px`);assert.ok(layout.calibrationOverflow<=1,`calibration panel overflows ${width}px by ${layout.calibrationOverflow}px`);assert.ok(layout.logOverflow<=1,`daily log overflows ${width}px by ${layout.logOverflow}px`);}await page.setViewportSize({width:1280,height:900});
