...
test("training-block review derives time, shows only logged evidence, and confirms every action",{timeout:60_000},async()=>{
  ...
  await page.goto("/discover.html#planWorkspace",{waitUntil:"domcontentloaded"});await page.locator("#trainingBlockReview").waitFor({state:"visible"});await page.waitForFunction(()=>globalThis.document.querySelector("#trainingBlockWorkoutCount")?.textContent==="0 / 2");
