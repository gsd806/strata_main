...
test("activation continuity uses explicit, revision-safe account choices",{timeout:90_000},async t=>{
  ...
  try{
    await t.test("a homepage week survives account and verification pages, then is claimed explicitly",async()=>{
      ...
      await page.goto("/",{waitUntil:"domcontentloaded"});await page.locator("#preview > summary").click();await page.locator("#quickPreviewEquipment").waitFor({state:"visible"});await page.waitForFunction(()=>!globalThis.document.querySelector("#quickPreviewEquipment").disabled);
      ...
      assert.equal(await page.locator("#compareDevicePlan").isHidden(),true,"an empty account has no conflicting week to compare");assert.equal(await page.locator("#devicePlanConfirmLabel").isHidden(),true);assert.match(await page.locator("#claimDevicePlan").textContent(),/Save week to my account/i);
