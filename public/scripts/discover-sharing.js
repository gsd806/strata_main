...
(function(root,factory){
  ...
  function createSharing({state,document,navigator,urlApi,fileCtor,labels,exerciseById,titleCase,personalResult,personalLabel,comparisonWinner,showToast}){
    function cardLines(kind,id){
      ...
      if(kind==="comparison"){const exercises=state.compare.map(exerciseById).filter(Boolean),verdict=comparisonWinner(exercises);return{eyebrow:"Exercise comparison",title:exercises.map((exercise)=>exercise.name).join(" vs. "),score:verdict.winner?String(verdict.winner.score):"—",scoreLabel:verdict.winner?"LEADING FITSCORE":"NO UNIVERSAL WINNER",lines:exercises.map((exercise)=>`${exercise.score} FitScore · ${personalLabel(personalResult(exercise))} — ${exercise.name}`),footer:"Compare the trade-offs, not just the score"};}
      const top=state.recommendations.slice(0,5);return{eyebrow:"Recommended exercises",title:`${titleCase(state.preferences.goal)} selection`,score:String(top[0]?.result.match||"—"),scoreLabel:"Match for you",lines:top.map(({exercise,result},index)=>`${index+1}. ${exercise.name} — ${result.match}% match`),footer:`${state.preferences.days} days · ${state.preferences.level} · community ratings separate`};
    }
    ...
    async function shareCard(kind,id=null){
      try{
        ...
        if(typeof fileCtor==="function"&&navigator.share){const file=new fileCtor([blob],filename,{type:"image/png"});if(navigator.canShare?.({files:[file]})){try{await navigator.share({files:[file],title:`STRATA ${kind} card`,text:"Exercise details from STRATA"});return;}catch(error){if(error.name==="AbortError")return;}}}
