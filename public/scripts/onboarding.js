...
(()=>{
  ...
  async function init(){
    ...
    try{
      if(!exercises.length){
        const response=await fetch("/exercises.json?v=7.8.6");if(!response.ok)throw new Error("The exercise library is unavailable. Reconnect and retry.");exercises=await response.json();
      }
