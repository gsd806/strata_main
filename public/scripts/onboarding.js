...
(()=>{
  ...
  let exercises=[],user=null,csrf="",revision=0,original=null,preview=null,ready=false,busy=false,profileKey="",previousDownload=null;
  ...
  function allowEditing(){
    profileKey=`strata_setup_v1:user:${user.id}`;restoreProfile();ready=true;$("setupFields").disabled=false;
    $("accountMode").textContent=`Strata+ · ${user.name||"Your account"}. Your saved week syncs across devices.`;
    $("retrySetup").hidden=true;status("");
  }
  function requirePlus(account){
    if(!account?.user?.id)throw new Error("Sign in to Strata+ to set up your week.");
    if(account.user.discovery?.active!==true){ready=false;$("setupFields").disabled=true;$("saveWeek").disabled=true;throw new Error("Set up my week is included in Strata+. Open Strata+ to restore access; your plan has not changed.");}
  }
  ...
  async function init(){
    ...
    try{
      if(!exercises.length){
        const response=await fetch("/exercises.json?v=7.1.1");if(!response.ok)throw new Error("The exercise library is unavailable. Reconnect and retry.");exercises=await response.json();
      ...
      const account=await request("/api/plan",{cache:"no-store"});requirePlus(account);
  ...
  $("setupForm").addEventListener("submit",async event=>{
    ...
    busy=true;$("setupFields").disabled=true;
    ...
    finally{busy=false;$("setupFields").disabled=!ready;}
  });
  ...
  $("saveWeek").addEventListener("click",async()=>{
    ...
    try{
      ...
      $("saveControls").hidden=true;$("openPlanner").hidden=false;status("Saved to your account. Open your planner to make it yours.");$("openPlanner").focus();
    ...
    finally{busy=false;$("saveWeek").disabled=!ready;$("setupFields").disabled=!ready;}
  });
