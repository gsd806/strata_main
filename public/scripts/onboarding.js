...
(()=>{
  ...
  async function init(){
    ...
    try{
      if(!exercises.length){
        const response=await fetch("/exercises.json?v=8.0.0");if(!response.ok)throw new Error("The exercise library is unavailable. Reconnect and retry.");exercises=await response.json();
      }
