...
(function(root,factory){
  ...
  function libraryMarkup(items,{selectedDay,visibleLimit,pageSize}){
    ...
    if(!items.length)return '<div class="library-empty"><strong>No matching movements</strong><span>Try another search or choose a different muscle group.</span></div>';
  ...
  function planConflictSummaryMarkup({plan,days,restDays=[],exerciseById,movementCount}){
    const rows=days.map(day=>{
      ...
      const detail=items.length?items.map(item=>{const exercise=exerciseById(item.exerciseId);return`${escapeHtml(exercise?.name||"Unknown movement")} <span>${escapeHtml(item.sets)} × ${escapeHtml(item.reps)}</span>`;}).join(", "):"No movements";
      return `<li><strong>${escapeHtml(day)}${restDays.includes(day)?" · recovery":""}</strong><p>${detail}</p></li>`;
    }).join("");
    return `<p class="plan-conflict-total">${movementCount} movement${movementCount===1?"":"s"} · ${restDays.length} rest days</p><ul>${rows}</ul>`;
  }
