...
(function(root,factory){
  ...
  function createCatalog({state,core,labels,preferenceOptions,limitationOptions,movementBoardLimit,desktopPageSize,mobilePageSize,ratingsRefreshInterval,document,window,element,escapeHtml,exerciseById,titleCase,personalResult,aggregateFor,api,getGeneration,saveMovementBoard,showToast,openDetail,openComparison}){
    ...
    function renderMovementBoard({message=""}={}){
      ...
      const filled=exercises.map((exercise,index)=>{const personal=personalResult(exercise),group=labels[exercise.group]||titleCase(exercise.group),fit=personal.eligible?`${personal.match}% match`:"Excluded";return`<article class="movement-board-item"><button class="movement-board-open" data-open-detail="${exercise.id}" type="button" aria-label="Inspect ${escapeHtml(exercise.name)}, ${escapeHtml(group)}, ${escapeHtml(exercise.equipment)}, ${escapeHtml(fit)}"><span>${String(index+1).padStart(2,"0")}</span><span><strong>${escapeHtml(exercise.name)}</strong><small>${escapeHtml(group)} · ${escapeHtml(exercise.equipment)}</small></span><b class="${personal.eligible?"":"is-excluded"}">${escapeHtml(fit)}</b></button><button class="movement-board-remove" data-toggle-shortlist="${exercise.id}" type="button" aria-label="Remove ${escapeHtml(exercise.name)} from your decision board">×</button></article>`;}).join("");
      ...
      element("movementBoardStatus").textContent=message||(count===0?"Nothing saved yet.":count===1?"One movement saved. Add another to compare.":`${count} movements saved. Ready to compare.`);
    }
    ...
    function renderExplorer(){
      ...
      const savedEmpty=!items.length&&state.collection==="saved"&&!state.shortlist.length;element("emptyStateTitle").textContent=savedEmpty?"Your decision board is empty.":"No exercise matches every filter.";element("emptyStateDetail").textContent=savedEmpty?"Save up to four movements from recommendations or the library, then return here to review them together.":"Try clearing the search or one of the exercise filters.";
    ...
    function toggleMovementBoard(id){
      ...
      if(removing)state.shortlist.splice(index,1);else if(state.shortlist.length>=movementBoardLimit){showToast(`Your decision board holds ${movementBoardLimit} movements. Remove one before saving another.`);return false;}else state.shortlist.push(id);
      const persisted=saveMovementBoard(),message=removing?`${exercise.name} removed from your decision board.`:`${exercise.name} saved${persisted?" on this device":" for this visit"}.`;renderMovementBoard({message});renderRecommendations();renderExplorer();if(state.activeExercise===id&&element("detailDialog")?.open)openDetail(id);showToast(message);return true;
    }
