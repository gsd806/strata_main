(function(){
  ...
  async function initialize(){
    ...
    try{const response=await fetch("/exercises.json?v=7.8.6");if(response.ok){const catalog=await response.json();state.catalog=new Map(catalog.map((item)=>[item.id,item]));}}catch{/* Exercise IDs remain usable if the public catalog is unavailable. */}
