...
STRATA is an evidence-informed workout index with server-backed, email-verified accounts, a private Strata+ studio, and weekly, community, and monthly workout planning. It includes 320 resistance-training exercises—including 71 bodyweight options—across 8 muscle groups and 26 sub-muscle targets. Build 8.0.1 is an installable Progressive Web App (PWA) with Training Memory, Resend-powered account email, Paddle-powered Strata+ subscriptions, and a private owner dashboard.

**Build 8.0.1 is the individualized activity and deficit release for Personal training and calorie counting.** New profiles separate ordinary daily movement, usable minutes in the generated STRATA week, and other planned weekly activity so each contribution is counted once and explained. Maintenance starts from Mifflin–St Jeor, keeps the 2023 adult EER visible as a population cross-check, and can still adapt gradually from sufficiently complete intake and morning-weight history. Gentle and moderate deficits are based on planning weight and then conservatively capped; body-fat input can only add disclosed review restrictions, never raise maintenance. Stored profile versions 1–3 and existing weekly snapshots keep their prior calculation semantics until an explicit review. No database migration is needed. These are planning estimates, not measured metabolism, medical prescriptions, or guaranteed outcomes. See the [8.0.1 release guide](docs/release-8.0.1.md), [coaching methodology](docs/coaching-methodology.md), and [synthetic benchmark](qa/calibration-benchmark.js).
...
Build 8.0.1 separates browser files from private server code while preserving every public URL used by visitors, Paddle, Render, and installed PWAs:
