...
STRATA is an evidence-informed workout index with server-backed, email-verified accounts, a private Strata+ studio, and weekly, community, and monthly workout planning. It includes 320 resistance-training exercises—including 71 bodyweight options—across 8 muscle groups and 26 sub-muscle targets. Build 7.8.8 is an installable Progressive Web App (PWA) with Training Memory, Resend-powered account email, Paddle-powered Strata+ subscriptions, and a private owner dashboard.

**Build 7.8.8 is the guided coaching setup and $2.99 USD monthly release.** The profile is now a guided four-step surface with separate cards, compact mobile orientation, labeled known-exercise inputs, 200% text reflow, and stronger contrast. New checkout fails closed unless Paddle confirms the exact 299-cent USD monthly catalog item, while an explicit server-only allowlist can preserve existing subscribers on an earlier recurring price. See the [7.8.8 release guide](docs/release-7.8.8.md).
...
Build 7.8.8 separates browser files from private server code while preserving every public URL used by visitors, Paddle, Render, and installed PWAs:
