...
STRATA is an evidence-informed workout index with server-backed, email-verified accounts, a private Strata+ studio, and weekly, community, and monthly workout planning. It includes 320 resistance-training exercises—including 71 bodyweight options—across 8 muscle groups and 26 sub-muscle targets. Build 8.0.0 is an installable Progressive Web App (PWA) with Training Memory, Resend-powered account email, Paddle-powered Strata+ subscriptions, and a private owner dashboard.

**Build 8.0.0 is the personal training and calorie refinement release.** Calorie feedback aligns complete food logs with the exact morning-weight interval, uses bounded evidence from the preceding 42 dates, and adapts gradually across weeks. Recent credible weights inform calculations, uncertainty stays visible, and daily calorie and macro totals reconcile. Training adds separate balanced, strength, and hypertrophy goals; stable main exercises, realistic time budgets, direct-muscle coverage, and targets from comparable completed workouts. Food options scale ingredient quantities and disclose calorie or macro shortfalls. The diary accepts recent past-day corrections with their original targets, and saved weeks remain fixed until the next week or an explicit profile save. Existing legacy calorie semantics remain intact; no database migration is needed. These are improved planning calculations, not measured metabolism or guaranteed accuracy. See the [8.0.0 release guide](docs/release-8.0.0.md), [coaching methodology](docs/coaching-methodology.md), and [synthetic benchmark](qa/calibration-benchmark.js).
...
Build 8.0.0 separates browser files from private server code while preserving every public URL used by visitors, Paddle, Render, and installed PWAs:
