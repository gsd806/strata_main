...
STRATA is an evidence-informed workout index with server-backed, email-verified accounts, a private Strata+ studio, and weekly, community, and monthly workout planning. It includes 320 resistance-training exercises—including 71 bodyweight options—across 8 muscle groups and 26 sub-muscle targets. Build 7.10.0 is an installable Progressive Web App (PWA) with Training Memory, Resend-powered account email, Paddle-powered Strata+ subscriptions, and a private owner dashboard.

**Build 7.10.0 is the trend-informed energy planning release.** A newly created or explicitly reviewed-and-saved coaching profile uses schema version 3 and the sex-specific 2023 adult Dietary Reference Intake Estimated Energy Requirement equations; Mifflin–St Jeor resting energy and optional Cunningham lean-mass values remain visible cross-checks. Existing version-1 and version-2 profiles keep their exact prior resting-energy × legacy activity-factor calculation and do not calibrate until the member reviews and saves the profile, so deployment cannot silently reinterpret a stored answer. Members can optionally save a morning weight and explicitly mark an intake day complete. A version-3 weekly plan can use only the preceding 21 days—and only after at least 18 complete intake days plus 12 morning weights spanning at least 14 calendar days—to apply a robust, heavily shrunk, 25-kcal-rounded correction capped at 150 kcal. Starting estimate, Calibrating, Trend-informed, and Legacy profile states disclose the model and evidence, while the current week's targets stay fixed. Migration `005-coaching-calibration` adds the two nullable evidence fields with SQLite/Turso parity and preserves the distinction between legacy, incomplete, and complete logs. See the [7.10.0 release guide](docs/release-7.10.0.md) and [coaching methodology](docs/coaching-methodology.md).
...
Build 7.10.0 separates browser files from private server code while preserving every public URL used by visitors, Paddle, Render, and installed PWAs:
